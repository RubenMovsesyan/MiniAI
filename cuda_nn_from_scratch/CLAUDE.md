...
Guidance for Claude Code when working inside `cuda_nn_from_scratch/`.

## Project Overview

**Archived / historical.** This is the original from-scratch CUDA neural network: GPU math
primitives, forward and back propagation written by hand to learn how they work. It reaches
~97% MNIST test accuracy. It is **feature-complete for its purpose and no longer actively
developed** — active work happens in the sibling `pytorch/` directory. Kept for reference;
don't extend it unless explicitly asked.

Everything below (paths, `./build`, modules) is relative to **this directory**, not the repo
root. Run `./build` from here.
...
The PyTorch track lives in the sibling `../pytorch/` directory — unrelated to `./build`,
nothing here depends on it. See `../pytorch/docs/ml_framework.md`.
