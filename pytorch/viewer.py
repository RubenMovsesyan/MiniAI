"""Prediction viewer: correct vs incorrect panes + per-conv feature maps.
...
def show(model: nn.Module, loader: DataLoader, device: str = "cuda", shuffle: bool = True) -> None:
    ...
    def all_logits() -> np.ndarray:
        outs = [model(X[i:i + 1000].to(device)).cpu().numpy() for i in range(0, len(x), 1000)]
        return np.concatenate(outs, 0)

    pred = all_logits().argmax(1)
    correct_mask = pred == y
    correct_idx, wrong_idx = np.where(correct_mask)[0], np.where(~correct_mask)[0]
    if shuffle:
    ...
    @torch.no_grad()
    def infer(idx: int):
        lg = model(X[idx:idx + 1].to(device)).cpu().numpy().ravel()
        maps = [F.relu(acts[k])[0].cpu().numpy() for k in range(len(convs))]
        return lg, maps

    def bar_colors(p, t):
        return [(GREEN if k == t else RED) if k == p else (GREEN if k == t else BLUE) for k in range(10)]

    fig = plt.figure(figsize=(14, 9))
    conv_rows = [int(np.ceil(c.out_channels / NCOLS)) for c in convs]
    fig.suptitle(f"test accuracy: {len(correct_idx)}/{len(y)} = {correct_mask.mean():.2%}   "
                 f"({len(wrong_idx)} wrong)", fontsize=13)

    pane_sfigs = fig.subfigures(1, 2, wspace=0.04)
    panes = []
    for name, idxs, sf in [("Correct", correct_idx, pane_sfigs[0]), ("Incorrect", wrong_idx, pane_sfigs[1])]:
        ...
        panes.append({"name": name, "idx": idxs, "pos": 0, "img": ax_img, "bar": ax_bar,
                      "conv_axes": conv_axes, "btn": btn})
    ...
    fig._viz_buttons = [p["btn"] for p in panes]
